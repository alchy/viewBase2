...
Ukazuje čtyři věci, které spolu drží:
 ...
 - `graph.on("terminal_write", …)` + REST `/api/event` – push zvenčí:
...
def zapis(text: str) -> None:
    ...
    graph.terminal_write("konzole", text)
...
def prikaz(event) -> None:
    ...
    try:
        if cmd == "pomoc":
            ...
            graph.ensure_node(args[0], label="{name}", name=args[0])
            graph.ensure_edge(args[0], "srv-0")
            zapis(f"uzel {args[0]} připojen na srv-0")
        elif cmd == "smaz" and args:
            graph.remove_node(args[0])          # kaskádou padnou i hrany
            zapis(f"uzel {args[0]} odebrán")
        elif cmd == "barva" and len(args) == 2:
            graph.update_node(args[0], color=args[1])   # živá barva uzlu
            zapis(f"{args[0]} má barvu {args[1]}")
        elif cmd == "typ" and len(args) == 2:
            graph.update_node(args[0], type=args[1])    # živá změna typu
            graph.focus(args[0])
            zapis(f"{args[0]} je teď {args[1]}")
        elif cmd == "kde" and args:
            uzel = graph.node(args[0])                  # čtení stavu
...
graph.open_terminal(konzole, on_input=prikaz)
...
def push_zvenku(event) -> None:
    ...
    if isinstance(text, str):
        graph.terminal_write(getattr(event, "window_id", "log"), text)


# on() bere jméno eventu i handler (není dekorátor jako on_click)
graph.on("terminal_write", push_zvenku)
