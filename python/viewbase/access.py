...
class Acl:
    ...
    def __init__(self, hodnoty: Iterable[str] | None = None) -> None:
    ...
    def add(self, *names: str) -> "Acl":
        if self._set is None:
            self._set = set()
        self._set.update(principal(n) for n in names)
        return self
    ...
    def remove(self, *names: str) -> "Acl":
        ...
        for n in names:
            self._set.discard(principal(n))
        return self

    def set(self, names: Iterable[str] | None) -> "Acl":
        """Přepiš celé ACL (`None` = zpátky na dědění)."""
        self._set = ({principal(n) for n in names} if names is not None else None)
        return self
...
class Access:
    ...
    def __init__(self, see: Iterable[str] | None = None,
                 write: Iterable[str] | None = None) -> None:
        self.see = Acl(see)
        self.write = Acl(write)

    def effective_see(self, fallback: Iterable[str]) -> set[str]:
        """Kdo vidí: vlastní ACL, jinak zděděné (plocha → instance)."""
...
def reset_default() -> None:
    """Zpátky na `group:users` (testy, nový běh)."""
    global DEFAULT_ACCESS
